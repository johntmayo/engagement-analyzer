...
function gmailConfigStatus() {
  ...
  return {
    ...
    lookbackDays: Math.max(
      1,
      Math.min(365, Number(process.env.GMAIL_LOOKBACK_DAYS) || 180)
    ),
  };
}
...
export function getGmailIntegrationStatus() {
  ...
  return {
    ...
    notes: [
      ...
      'Mailbox activity remains a transparent signal. No points or composite score are assigned.',
    ],
  };
}
...
function dateForGmailQuery(value) {
...
function mailboxStartDate(mailbox, existingEvents, lookbackDays) {
  const latest = existingEvents
    .filter((event) => normalizeEmail(event.mailbox) === mailbox)
    .map((event) => Date.parse(event.occurred_at || ''))
    .filter(Number.isFinite)
    .sort((a, b) => b - a)[0];
  const fallback = Date.now() - lookbackDays * 86400000;
  // Re-read two days to tolerate delayed delivery and timestamp boundaries.
  return new Date(latest ? latest - 2 * 86400000 : fallback);
}
...
async function fetchMailboxEvents(mailbox, config, existingEvents) {
  ...
  const startDate = mailboxStartDate(
    mailbox,
    existingEvents,
    config.lookbackDays
  );
  const queryParts = [`after:${dateForGmailQuery(startDate)}`];
  if (
  ...
  // Engagement needs recent activity, not a complete email archive. A bounded
  // batch also keeps the initial four-mailbox pull within Gmail per-user quota.
...
export async function syncGmailMailbox() {
  ...
  for (const mailbox of status.mailboxes) {
    try {
      mailboxResults.push(
        await fetchMailboxEvents(mailbox, status, existingEvents)
      );
    } catch (error) {
      mailboxErrors.push({
        mailbox,
