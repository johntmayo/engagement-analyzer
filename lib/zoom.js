...
export async function storeZoomSessions(sessions) {
  const res = await fetch('/api/store-zoom', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ sessions }),
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || 'Zoom data could not be saved to Google Sheets');
  return data;
}
...
export async function getAllUsers(token) {
  ...
  do {
    ...
    const data = await zoomFetch(token, '/users', params);
